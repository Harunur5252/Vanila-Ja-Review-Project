...
# Reviews 
This is a simple review project using javaScript and create json api server where data comes from it to ui.

## Project Specifications

+ Created a simple UI(user interface).
+ Initial state have three buttons.
+ If click on grater than icon button then value will be increased and changed in ui on that time.
+ If click on less than icon then value will be decreased and changed in ui on that time.
+ If click on Surprise Me button then value will come randomly.


## Project Picture

![Reviews!](/src/images/review.PNG "Reviews")
